...
pub fn bellman_ford<T: Display + Eq + Clone>(graph: &mut Graph<T>, source: &T, target: &T) -> Option<i32> {
    graph.reset_nodes();
    let start = graph.node_by_id(source);
    None
}

mod tests {
    use crate::{core::{Graph, Node}, algorithms::{self, dijkstra}};
